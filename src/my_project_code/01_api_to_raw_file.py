...
try:
    ...
    # Fail the task hard so the workflow halts
...
# Write the raw JSON file with a UTC timestamp in the name
filename_ts = api_response_ts.strftime("%Y%m%dT%H%M%SZ")
filename = f"flights_{filename_ts}.json"
output_path = f"{raw_dir}/{filename}"

# Write via local /tmp then copy into the Volume — simplest reliable approach
local_tmp = f"/tmp/{filename}"
with open(local_tmp, "w") as f:
    json.dump(payload, f)

dbutils.fs.cp(f"file:{local_tmp}", output_path)
